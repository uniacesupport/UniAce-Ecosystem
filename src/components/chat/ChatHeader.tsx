...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  return (
    <header className="bg-slate-900 dark:bg-black text-white px-4 sm:px-6 py-4 pt-safe flex flex-col gap-3 shrink-0 border-b border-slate-800">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          {onToggleFullPage && (
            <button
              ...
              className="p-2 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 hover:text-emerald-100 rounded-xl transition-all flex items-center gap-1.5 text-xs font-extrabold border border-emerald-500/40 shrink-0 shadow-sm"
          ...
          <div className="w-9 h-9 sm:w-10 sm:h-10 bg-emerald-500 rounded-2xl flex items-center justify-center shrink-0 shadow-md shadow-emerald-500/20">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {/* Spark Counter */}
          <div className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800/80 border border-slate-700/80 text-[11px] font-bold">
            {hasUnlimitedSparks ? (
              <span className="text-emerald-400 flex items-center gap-1">
                <Zap size={12} fill="currentColor" /> Unlimited Sparks
              </span>
            ) : (
              <span className="text-amber-400 flex items-center gap-1">
                <Zap size={12} fill="currentColor" /> {sparksRemaining ?? 50} Sparks
              </span>
            )}
          </div>
          ...
          <button
            ...
            className="p-2 text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-colors flex items-center gap-1 text-xs font-semibold"
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-800/80 text-xs">
          ...
          {(['encouraging', 'socratic', 'strict', 'debate'] as AIPersonality[]).map(mode => {
            ...
            return (
              <button
                ...
                className={`px-2.5 py-1 rounded-lg text-[11px] font-bold transition-all flex items-center gap-1.5 whitespace-nowrap ${
                  isSelected
        ...
        <div className="flex items-center gap-2 text-[11px] font-medium text-slate-300">
          <span className="text-slate-400 text-[10px] uppercase font-bold">Topic Mastery:</span>
          <div className="w-16 sm:w-20 bg-slate-800 rounded-full h-2 overflow-hidden border border-slate-700">
            <div 
